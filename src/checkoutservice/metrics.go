...
// gRPC FullMethod (e.g., /hipstershop.CheckoutService/PlaceOrder), bounded
// by the protobuf service definition rather than client input.
var (
	...
	circuitBreakerStateChanges = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Number of circuit breaker state transitions, partitioned by upstream service and target state.",
			// Labeled by upstream circuit name (set in newCircuitBreaker)
			// and target state. Bounded by the number of upstream services.
		},
		[]string{"circuit", "to"},
	)
)
...
// The returned server can be Shutdown by the caller during graceful drain.
...
// (low cardinality, bounded by the protobuf service). Code label uses the
// canonical gRPC code string (OK, Unavailable, etc.).
